...
var Module = {
	...
	fairAmountByDaynce: function(cb) { //今年签约额增长走势
		getData(window.baseApiPath + "contract/fairAmountByDay", {
			beginDate: '',
			endDate: ''
		}, function(value) {
			var data = {
	...
	thisFairAudience: function(cb) { //本次展会签到人数
		getData(window.baseApiPath + "fairAudience/thisFairAudience", {
//			beginDate: '',
//			endDate: ''
		}, function(value) {
			var data = {
	...
	fairAmount: function(cb) { //历年签约额增长走势
		getData(window.baseApiPath + "contract/fairAmount", {
//			beginDate: '',
//			endDate: ''
		}, function(value) {
			var data = {
	...
	wineSales: function(cb) { //参展酒类签约额走势接口
		getData(window.baseApiPath + "contract/wineSales", {
//			beginDate: '',
//			endDate: ''
		}, function(value) {
			var data={
	...
	todayFairAudience: function(cb) { //签到人数
		getData(window.baseApiPath + "fairAudience/todayFairAudience", {
//			beginDate: '',
//			endDate: ''
		}, function(value) {
	...
	todayFairAmount: function(cb) { //共签约
		getData(window.baseApiPath + "contract/todayFairAmount", {
//			beginDate: '',
//			endDate: ''
		}, function(value) {
	...
	categoryCode: function(cb,code) { //本次展会签约额排行
		if(code){
			getData(window.baseApiPath + "contract/contractBycategoryCode", {
				industry_category_code : code
			}, function(value) {
				var data = [];
				value.data.forEach(function(v,i) {
					data[i] = {};
			...
			getData2(window.baseApiPath + "contract/contractBycategoryCode", function(value) {
				var data = [];
				value.data.forEach(function(v,i) {
					data[i] = {};
					data[i].index = (i+1);
	...
	lastFairAudience: function(cb) { //本次展会签到人数
		getData(window.baseApiPath + "fairAudience/lastFairAudience", {
			pageIndex:'1',
			pageSize: '15'
		}, function(value) {
			var data = [];
