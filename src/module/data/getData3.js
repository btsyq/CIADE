...
var Module = {
	...
	capacityByDay: function(cb) {
		...
		getData(window.baseApiPath + "capacityStatistics/capacityByDay", {}, function(value) {
			var data = {
				quantity: [],
				province: []
			}
	...
	yieldStatisticsSales: function(cb) {
		getData(window.baseApiPath + "yieldStatistics/sales", {}, function(value) {
			cb(value)
		});
	...
	yieldStatisticsWineTaste: function(cb) {
		getData(window.baseApiPath + "yieldStatistics/wineTaste", {}, function(value) {
			var data = [];
	...
	yieldStatisticsWineProduction: function(cb, code) {
		...
		//console.log(code)
		getData(window.baseApiPath + "yieldStatistics/wineProduction", { category_code: _code }, function(value) {
			var data = {
				year: [],
				quantity: [],
				rate_of_growth: []
	...
	yieldStatisticsEcommerceSales: function(cb) {
		getData(window.baseApiPath + "yieldStatistics/ecommerceSales", {}, function(value) {
			var data = {
				yname: [],
				winename: [],
				data: []
	...
	yieldStatisticsRegionSalesMap: function(cb, code) { //参展企业数量走势数据
		getData(window.baseApiPath + "yieldStatistics/regionSales", { oper_type_code: code }, function(value) {
			var data = [];
			value.data.forEach(function(v, i) {
				data[i] = {};
				data[i].id = v.code;
	...
	yieldStatisticsRegionSales: function(cb, code) {
		getData(window.baseApiPath + "yieldStatistics/regionSales", { oper_type_code: 0 }, function(value) {
			var data = []
			value.data.forEach(function(v, i) {
				data.push({
					index: i + 1,
	...
	yieldStatisticsRegionSales2: function(cb, code) {
		getData(window.baseApiPath + "yieldStatistics/regionSales", { oper_type_code: 1 }, function(value) {
			var data = []
			value.data.forEach(function(v, i) {
				data.push({
					index: i + 1,
